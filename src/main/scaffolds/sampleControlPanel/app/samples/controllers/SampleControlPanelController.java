...
@AuthRequired(loginCall = "samples.controllers.routes:SampleController:login", 
    usergroups = { SampleConstants.USERGROUP_ID_ADMIN })
public class SampleControlPanelController extends BasePageController {
     ...
     */
    public Result home(Http.Request request) throws Exception {
        return ok(render(request, VIEW_HOME));
    }
     ...
     */
    public Result usergroups(Http.Request request) throws Exception {
        ...
        return ok(render(request, VIEW_USERGROUPS, (Object) allUsergroups));
    }
    ...
    public Result createUsergroup(Http.Request request) throws Exception {
        ...
        Html html = render(request, VIEW_CREATE_USERGROUP, form);
        return ok(html);
    }
     ...
     */
    public Result createUsergroupSubmit(Http.Request request) throws Exception {
        Form<FormCreateEditUsergroup> form = formFactory.form(FormCreateEditUsergroup.class).bindFromRequest(request);
        if (form.hasErrors()) {
            Html html = render(request, VIEW_CREATE_USERGROUP, form);
            return ok(html);
        }
        FormCreateEditUsergroup formData = form.get();
        UserGroupBo bo = UserGroupBo.newInstance(formData.getId()).setDescription(formData.getDescription());
        IUserDao dao = getRegistry().getBean(IUserDao.class);
        ...
        if (result.getStatus() == DaoOperationStatus.SUCCESSFUL) {
            return responseRedirect(
                    ...
                    VIEW_USERGROUPS, calcMessages(request).at("msg.create_usergroup.done", bo.getId()));
        } else {
            return responseRedirect(
                    samples.controllers.routes.SampleControlPanelController.usergroups(),
                    VIEW_USERGROUPS, SampleConstants.FLASH_MSG_PREFIX_WARNING
                            + calcMessages(request).at("msg.create_usergroup.failed", bo.getId()));
        }
    }
     ...
     */
    public Result editUsergroup(Http.Request request, String id) throws Exception {
        IUserDao dao = getRegistry().getBean(IUserDao.class);
        ...
        if (bo == null) {
            return responseRedirect(
                    ...
                    VIEW_USERGROUPS, SampleConstants.FLASH_MSG_PREFIX_ERROR
                            + calcMessages(request).at("error.usergroup.not_found", id));
        }
        Form<FormCreateEditUsergroup> form = formFactory.form(FormCreateEditUsergroup.class)
                .fill(FormCreateEditUsergroup.newInstance(bo));
        Html html = render(request, VIEW_EDIT_USERGROUP, form);
        return ok(html);
    }
     ...
     */
    public Result editUsergroupSubmit(Http.Request request, String id) throws Exception {
        IUserDao dao = getRegistry().getBean(IUserDao.class);
        UserGroupBo bo = dao.getUserGroup(id);
        if (bo == null) {
            return responseRedirect(
        ...
        if (result.getStatus() == DaoOperationStatus.SUCCESSFUL) {
            return responseRedirect(
                    samples.controllers.routes.SampleControlPanelController.usergroups(),
                    VIEW_USERGROUPS, calcMessages(request).at("msg.edit_usergroup.done", bo.getId()));
        } else {
            return responseRedirect(
                    samples.controllers.routes.SampleControlPanelController.usergroups(),
                    VIEW_USERGROUPS, SampleConstants.FLASH_MSG_PREFIX_WARNING
                            + calcMessages(request).at("msg.edit_usergroup.failed", bo.getId()));
        }
    }
     ...
     */
    public Result deleteUsergroup(Http.Request request, String id) throws Exception {
        IUserDao dao = getRegistry().getBean(IUserDao.class);
        UserGroupBo bo = dao.getUserGroup(id);
        if (bo == null) {
            return responseRedirect(
            ...
            return responseRedirect(
                    samples.controllers.routes.SampleControlPanelController.usergroups(),
                    VIEW_USERGROUPS, SampleConstants.FLASH_MSG_PREFIX_ERROR + calcMessages(request)
                            .at("error.usergroup.cannot_delete_sytem_usergroup", id));
        }
        Html html = render(request, VIEW_DELETE_USERGROUP, UserGroupModel.newInstance(bo));
        return ok(html);
    }
     ...
     */
    public Result deleteUsergroupSubmit(Http.Request request, String id) throws Exception {
        IUserDao dao = getRegistry().getBean(IUserDao.class);
        UserGroupBo bo = dao.getUserGroup(id);
        if (bo == null) {
            return responseRedirect(
        ...
        if (result.getStatus() == DaoOperationStatus.SUCCESSFUL) {
            return responseRedirect(
                    samples.controllers.routes.SampleControlPanelController.usergroups(),
                    VIEW_USERGROUPS, calcMessages(request).at("msg.delete_usergroup.done", bo.getId()));
        } else {
            return responseRedirect(
                    samples.controllers.routes.SampleControlPanelController.usergroups(),
                    VIEW_USERGROUPS, SampleConstants.FLASH_MSG_PREFIX_WARNING
                            + calcMessages(request).at("msg.delete_usergroup.failed", bo.getId()));
        }
    }
     ...
     */
    public Result users(Http.Request request) throws Exception {
        IUserDao dao = getRegistry().getBean(IUserDao.class);
        UserModel[] allUsers = UserModel.newInstances(dao.getAllUsers());

        // Note: cast to Object to make sure the whole array is treated as a
        // single parameter value!
        return ok(render(request, VIEW_USERS, (Object) allUsers));
    }
    ...
    public Result createUser(Http.Request request) throws Exception {
        ...
        Html html = render(request, VIEW_CREATE_USER, form);
        return ok(html);
    }
     ...
     */
    public Result createUserSubmit(Http.Request request) throws Exception {
        Form<FormCreateEditUser> form = formFactory.form(FormCreateEditUser.class).bindFromRequest(request);
        if (form.hasErrors()) {
            Html html = render(request, VIEW_CREATE_USER, form);
            return ok(html);
        }
        ...
        if (result.getStatus() == DaoOperationStatus.SUCCESSFUL) {
            return responseRedirect(samples.controllers.routes.SampleControlPanelController.users(),
                    VIEW_USERS, calcMessages(request).at("msg.create_user.done", bo.getUsername()));
        } else {
            return responseRedirect(samples.controllers.routes.SampleControlPanelController.users(),
                    VIEW_USERS, SampleConstants.FLASH_MSG_PREFIX_WARNING
                            + calcMessages(request).at("msg.create_user.failed", bo.getUsername()));
        }
    }
     ...
     */
    public Result editUser(Http.Request request, String username) throws Exception {
        IUserDao dao = getRegistry().getBean(IUserDao.class);
        UserBo bo = dao.getUser(username);
        if (bo == null) {
            return responseRedirect(samples.controllers.routes.SampleControlPanelController.users(),
                    VIEW_USERS, SampleConstants.FLASH_MSG_PREFIX_ERROR
                            + calcMessages(request).at("error.user.not_found", username));
        }
        Form<FormCreateEditUser> form = formFactory.form(FormCreateEditUser.class)
                .fill(FormCreateEditUser.newInstance(bo));
        Html html = render(request, VIEW_EDIT_USER, form);
        return ok(html);
    }
     ...
     */
    public Result editUserSubmit(Http.Request request, String username) throws Exception {
        IUserDao dao = getRegistry().getBean(IUserDao.class);
        UserBo bo = dao.getUser(username);
        if (bo == null) {
            return responseRedirect(samples.controllers.routes.SampleControlPanelController.users(),
        ...
        if (result.getStatus() == DaoOperationStatus.SUCCESSFUL) {
            return responseRedirect(samples.controllers.routes.SampleControlPanelController.users(),
                    VIEW_USERS, calcMessages(request).at("msg.edit_user.done", bo.getUsername()));
        } else {
            return responseRedirect(samples.controllers.routes.SampleControlPanelController.users(),
                    VIEW_USERS, SampleConstants.FLASH_MSG_PREFIX_WARNING
                            + calcMessages(request).at("msg.edit_user.failed", bo.getUsername()));
        }
    }
    ...
    public Result deleteUser(Http.Request request, String username) throws Exception {
        ...
        if (bo == null) {
            ...
            return responseRedirect(samples.controllers.routes.SampleControlPanelController.users(),
                    VIEW_USERS, SampleConstants.FLASH_MSG_PREFIX_ERROR
                            + calcMessages(request).at("error.user.cannot_delete_sytem_user", username));
        }
        Html html = render(request, VIEW_DELETE_USER, UserModel.newInstance(bo));
        return ok(html);
    }
     ...
     */
    public Result deleteUserSubmit(Http.Request request, String username) throws Exception {
        IUserDao dao = getRegistry().getBean(IUserDao.class);
        UserBo bo = dao.getUser(username);
        if (bo == null) {
            return responseRedirect(samples.controllers.routes.SampleControlPanelController.users(),
        ...
        if (result.getStatus() == DaoOperationStatus.SUCCESSFUL) {
            return responseRedirect(samples.controllers.routes.SampleControlPanelController.users(),
                    VIEW_USERS, calcMessages(request).at("msg.delete_user.done", bo.getUsername()));
        } else {
            return responseRedirect(samples.controllers.routes.SampleControlPanelController.users(),
                    VIEW_USERS, SampleConstants.FLASH_MSG_PREFIX_WARNING
                            + calcMessages(request).at("msg.delete_user.failed", bo.getUsername()));
        }
    }
}
