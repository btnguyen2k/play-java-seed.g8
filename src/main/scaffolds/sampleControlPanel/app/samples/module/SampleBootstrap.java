...
/**
 * Bootstrap sample Control Panel.
 *
 * @author Thanh Nguyen <btnguyen2k@gmail.com>
 * @since template-v2.6.r5
 */
public class SampleBootstrap {
    private Logger.ALogger LOGGER = Logger.of(SampleBootstrap.class);
     ...
     */
    @Inject
    public SampleBootstrap(ApplicationLifecycle lifecycle, Provider<IRegistry> registry) {
    ...
    private void init() throws Exception {
        ...
        try (Connection conn = ds.getConnection()) {
            if (!tableExists(conn, JdbcUserDao.TABLE_USER)) {
                ...
                conn.createStatement().execute("CREATE TABLE " + JdbcUserDao.TABLE_USER + "(uname VARCHAR(64), udata CLOB, PRIMARY KEY (uname))");
            }

            if (!tableExists(conn, JdbcUserDao.TABLE_USERGROUP)) {
                LOGGER.warn("Creating table [" + JdbcUserDao.TABLE_USERGROUP + "]...");
                conn.createStatement().execute("CREATE TABLE " + JdbcUserDao.TABLE_USERGROUP + "(gid VARCHAR(64), gdata CLOB, PRIMARY KEY (gid))");
            }
        }
        ...
        if (userAdmin == null) {
            ...
            userAdmin
                .setGroupId(SampleConstants.USERGROUP_ID_ADMIN)
                .setEmail("admin@localhost")
                .setFullname("Administrator")
                .setPassword(UserUtils.encryptPassword(pwd));
            dao.create(userAdmin);
            LOGGER.warn("Created user [" + SampleConstants.USERGROUP_ID_ADMIN + "] with password [" + pwd + "]");
        }
    }
